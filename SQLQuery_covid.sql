...
SELECT top 5 new_cases as "New Cases",location as Country
FROM Covid..deaths
WHERE continent IS NOT NULL 
order by new_cases
DESC
...
CREATE VIEW CTE_Vaccs_count AS
...
------------------------
CREATE VIEW Covid_infections AS
SELECT date, SUM(new_cases) infectionPerDay, SUM(new_deaths) AS deathsPerDay , SUM(new_deaths)/SUM(new_cases)*100 AS DeathPercentage
FROM Covid..deaths
WHERE continent IS NOT NULL
GROUP BY date 
...
CREATE VIEW Highest_death_rate
AS
